...
class HTTPClient:
    ...
    def perform(self):
        ...
        error_code = 0
        reponse = [{'na' : 0}]
        while error_code != 200 and trial_time < 100:
            trial_time += 1
            try:
                time.sleep(1)
                handler = urllib.request.urlopen(request)
                error_code = handler.getcode()
                ...
                time.sleep(1)
                error_code = e.code
                print("[", self.url,"]", error_code, ": ", e.read().decode("utf-8", "ignore"), sep = "")

        if response == [{'na' : 0}]:
            print("\033[91m", "[ERROR] Please remember to try ", self.url, " again.", "\033[0m", sep = "")
