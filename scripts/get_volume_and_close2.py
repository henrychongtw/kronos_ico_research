...
def get_volume_and_close(target_symbol_name, targetdate, api, filepath):
    ...
    time_end = datetime.date(2018, 12, 31) 
    ...
    if len(ohlcv_historical) > 0:
        ...
        for ohlcv in ohlcv_historical:
            if datetime.datetime.strptime(ohlcv["time_period_start"][0:10], "%Y-%m-%d") == time_start:
                ...
                while datetime.datetime.strptime(ohlcv["time_period_start"][0:10], "%Y-%m-%d") != time_start:
                    ...
                    time_start = datetime.datetime.strptime(((time_start + datetime.timedelta(days = 1))).strftime("%Y-%m-%d"), "%Y-%m-%d")
                    ...
                    if datetime.datetime.strptime(ohlcv["time_period_start"][0:10], "%Y-%m-%d") == time_start:
                        ...
                        filename = ''.join([time_start.strftime("%Y%m%d"), "_", target_symbol_name, "_empty.csv"])
        ...
        for dcount in range(0, 4):
            ...
            csv_file.close()
            print("[FINISH]: ", target_symbol_name, " in ", time_start.strftime("%Y%m%d"), ". (X)", sep = "")
...
def main(argv):
    # Parameter setting --------
    day_start_str = "20180801"
    day_end_str = "20180810"
    ...
    filepath = "/mnt/c/henryshawn/marketdata/ICO_NXT"
    symbol_id = ""
    ...
    target_exchange_list = ["HITBTC","POLONIEX","BITTREX","BITCOINID","NXT_IDR", "LIVECOIN", "OKEX", "CCEX"]
    target_instrument_list = ["SPOT"]
    target_asset_list = ["NXT_BTC", "NXT_USDT", "NXT_ETH" ]
    ...
    while True:
        ...
        targetdate = datetime.datetime.strptime(((targetdate + datetime.timedelta(days = 4))).strftime("%Y-%m-%d"), "%Y-%m-%d")
